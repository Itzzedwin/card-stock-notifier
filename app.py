...
def run_check(product, config):
    ...
    # Same guard the CLI/Actions checker uses: a target whose URL still has an
    # unresolved {SECRET} placeholder is skipped with a status that says so,
    # rather than fetching a literal "{API_KEY}" URL and reporting a 403.
    missing = core.SECRET_RE.findall(core.resolve_url(product["url"]))
    if missing:
        with lock:
            state = load_state()
            prev = state.get(product["id"], {})
            state[product["id"]] = {
                "status": "missing_secret", "error": None,
...
def api_test_notification():
    ...
    sent = core.notify(config, {
        ...
        "url": "https://itzzedwin.github.io/card-stock-notifier/",
    })
    if not sent:
